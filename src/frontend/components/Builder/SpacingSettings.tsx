...
export interface SpacingSettingsProps {
  ...
  /**
   * Optional leading copy explaining the panel. The onboarding modal passes a
   * first-run blurb; the Settings subpage passes nothing (its title carries
   * the framing).
   */
  intro?: ReactNode;
}
...
/**
 * Presentational spacing-configuration body shared by the first-run
 * `BuilderSetupModal` and the Settings > Spacing subpage. Reads/writes
 * everything through `useUiStore` / `useDesignStore`, so both hosts stay in
 * sync — there is no local copy of the config.
 */
